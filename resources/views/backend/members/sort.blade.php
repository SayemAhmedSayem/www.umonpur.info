...
<div class="row align-items-center justify-content-between g-3 mb-4">
   ...
   ?>
              <li class="nav-item"><a class="nav-link" aria-current="page" href="{{route('members.index')}}">All <span class="text-700 fw-semi-bold">({{$all}})<span></a></li>
              @foreach(App\Models\Role::All() as $role)
          <li class="nav-item"><a class="nav-link {{$role->id==$id ? 'active' : ''}}"  href="{{route('member-sort', $role->id)}}">{{$role->name ?? ''}} <span class="text-700 fw-semi-bold">
