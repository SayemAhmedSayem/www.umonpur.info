...
if (!function_exists('current_month_income')) {
    function current_month_income() {
        ...
        $monthly_income = \App\Models\Billing\Transaction::selectRaw("IFNULL(SUM(amount),0) as total")
            ->where("dr_cr", "cr")
            ->where("company_id", $company_id)
...
if (!function_exists('current_month_expense')) {
    function current_month_expense() {
        ...
        $monthly_expense = \App\Models\Billing\Transaction::selectRaw("IFNULL(SUM(amount),0) as total")
            ->where("dr_cr", "dr")
            ->where("company_id", $company_id)
            ->whereMonth("trans_date", $month)
            ->whereYear("trans_date", $year)
...
if (!function_exists('create_payment_method')) {
    function create_payment_method($methodName, $company_id) {
        $payment_method = \App\Models\Billing\PaymentMethod::where('name', $methodName)->where("company_id", $company_id);
        if ($payment_method->exists()) {
            return $payment_method->first()->id;
        } else {
            $payment_method             = new \App\Models\Billing\PaymentMethod();
...
if (!function_exists('update_stock')) {
    function update_stock($product_id) {
        ...
        //Update Stock
        $stock = \App\Models\Billing\Stock::where("product_id", $product_id)->where("company_id", $company_id)->first();
        if ($stock) {
